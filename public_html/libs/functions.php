...
function get_model_filepath($app, $module_name)
{
    return SERVER_ROOT . "apps/$app/modules/$module_name/" . $module_name . '_Model.php';
}

function build_url_search_van_ban($keywords = '', $v_cqbh = '', $v_lvtk = '', $v_loai_vb = '')
{
    if (check_file_htaccess())
    {
        $url       = SITE_ROOT . "van-ban-chi-dao-dieu-hanh";
        $start_get = '?';
    }
    else
    {
        $url       = SITE_ROOT . "index.php?url=frontend/frontend/dsp_all_van_ban_phap_quy";
        $start_get = '?';
    }
    $url .= $start_get . "txt_title=$keywords&sel_cqbh=$v_cqbh&sel_lvtk=$v_lvtk&sel_loai_vb=$v_loai_vb";
    return $url;
}
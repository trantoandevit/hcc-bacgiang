...
// end func daysInMonth
    ...
    public static function weeksInMonth($month = '', $year = '') {
        ...
        $FDOM = firstOfMonthWeekday($month, $year);
        ...
        $first_week_days %= 7;
        return (ceil((daysInMonth($month, $year) - $first_week_days) / 7) + $weeks);
    }
...
// end func weeksInMonth
    ...
    public static function firstOfMonthWeekday($month = '', $year = '') {
        ...
        return(dayOfWeek('01', $month, $year));
    }
    ...
    public static function getCalendarYear($year = '', $format = '%d%m%Y') {
        ...
        for ($curr_month = 0; $curr_month <= 11; $curr_month++) {
            $year_array[$curr_month] =
                    getCalendarMonth(sprintf('%02d', $curr_month + 1), $year, $format);
        }
...
//} // end class Date_Calc
    ...
    public static function NextnDay($n = 1, $day = '', $month = '', $year = '', $format = '%d%m%Y') {
        ...
        if (($format != '%d%m%Y') and ($format != '%m%d%Y') and ($format != '%Y%m%d') && ($format != '%d-%m-%Y') and ($format != '%m-%d-%Y') and ($format != '%Y-%m-%d')) {
            $format = '%d%m%Y';
        }
        ...
        if ($n > 0) {
            for ($i = 1; $i <= $n; $i++) {
                $tmp_day = self::NextDay(self::cutDay($tmp_day), self::cutMonth($tmp_day), self::cutYear($tmp_day));
            }
        } elseif ($n < 0) {
            for ($i = -1; $i >= $n; $i--) {
                $tmp_day =self:: PrevDay(self::cutDay($tmp_day), self::cutMonth($tmp_day), self::cutYear($tmp_day));
            }
        }
        return self::dateFormat(self::cutDay($tmp_day), self::cutMonth($tmp_day), self::cutYear($tmp_day), $format);
    }
...
//end func cutYear
    ...
    /**
     * Kiem tra chuoi co phai la ngay thang
     * @param string $str_date  chuoi can kiem tra
     * @param string $format Dinh dang chuoi vao ban dau
     * @return bool true la dung, sai la false
     */
    
    public static function validateDate($str_date, $format = 'Y-m-d H:i:s')
    {

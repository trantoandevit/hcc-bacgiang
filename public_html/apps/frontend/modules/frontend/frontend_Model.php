...
//         get array t_ps_citizen_tmp by username
    ...
    ################## Văn bản pháp quy####################
    public function qry_all_doc($condition)
    {
        return $this->db->getAll("SELECT *,(select count(PK_VAN_BAN) from t_ps_van_ban  ORDER BY C_NGAY_BAN_HANH DESC) as TOTAL_RECORD FROM t_ps_van_ban WHERE C_STATUS=1 $condition");
    }
    public function qry_all_new_doc($limit = _CONST_LIMIT_NEW_DOCUMENT)
    {
        return $this->db->getAll("SELECT * FROM t_ps_van_ban WHERE C_STATUS=1 ORDER BY C_NGAY_BAN_HANH DESC LIMIT $limit");
    }
    ...
    /**
     * Lấy thông tin chi tiết văn bản pháp quy
     * @param type $pk_vb mã văn bản
     * @return array
     */
    public function qry_single_doc($pk_vb)
    {
        return $this->db->getRow("SELECT
                                        vb.*,
                                        cqbh.C_NAME AS C_CQBH,
                                        lvtk.C_NAME AS C_LVTK

                                      FROM t_ps_van_ban vb
                                        LEFT JOIN t_ps_co_quan_ban_hanh cqbh
                                          ON vb.FK_CO_QUAN_BAN_HANH = cqbh.PK_CO_QUAN_BAN_HANH
                                        LEFT JOIN t_ps_linh_vuc_thong_ke lvtk
                                          ON vb.FK_LINH_VUC_VAN_BAN = lvtk.PK_LINH_VUC_VAN_BAN
                                      WHERE PK_VAN_BAN = ?
                                          AND vb.C_STATUS = 1",array($pk_vb));
    }
    public function qry_doc_involve_by_cqbh($pk_vb,$fk_cqbh)
    {
        return $this->db->getAll("SELECT * FROM t_ps_van_ban WHERE PK_VAN_BAN != $pk_vb
                                 AND FK_CO_QUAN_BAN_HANH = $fk_cqbh
                                 LIMIT 3");
    }
    public function qry_doc_involve_by_lvtk($pk_vb,$fk_lvtk)
    {
        return $this->db->getAll("SELECT * FROM t_ps_van_ban WHERE PK_VAN_BAN != $pk_vb
                                 AND FK_LINH_VUC_VAN_BAN = $fk_lvtk
                                 LIMIT 3");
    }
    /**
     * Lấy danh sách tập tin đính kèm của văn bản
     * @param int $doc_id Id của văn bản
     */
    public function qry_all_van_ban_attachment($doc_id)
    {
        return $this->db->GetAll("SELECT *
                                FROM t_ps_van_ban_dinh_kem
                                WHERE FK_VAN_BAN = ? ",array($doc_id));
    }
    #####################End van ban phap quy ##############################
}

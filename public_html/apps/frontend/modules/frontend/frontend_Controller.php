...
//        $this->_check_login_cookie();
    ...
    {
        $VIEW_DATA = $this->get_default_data();
        $this->view->active_menu_top = 'home-page';
        //Render
        $this->view->render('dsp_home_page', array(), $this->theme_code);
    }
    
    
    public function dsp_danh_gia_can_bo()
    {
        $VIEW_DATA = $this->get_default_data();
        $this->view->render('dsp_danh_gia_can_bo', $VIEW_DATA, $this->theme_code);
        exit();
    }
    public function dsp_list_thu_tuc_hanh_chinh()
    {
        $VIEW_DATA = $this->get_default_data();
        
        $this->view->render('dsp_list_thu_tuc_hanh_chinh', $VIEW_DATA, $this->theme_code);
        exit();
    }
    public function dsp_chi_tiet_thu_tuc_hanh_chinh()
    {
        $VIEW_DATA = $this->get_default_data();
        
        $this->view->render('dsp_chi_tiet_thu_tuc_hanh_chinh', $VIEW_DATA, $this->theme_code);
        exit();
    }
    public function dsp_search_ho_so()
    {
        $VIEW_DATA = $this->get_default_data();
        
        $this->view->render('dsp_search_ho_so', $VIEW_DATA, $this->theme_code);
        exit();
    }
    public function dsp_search_total()
    {
        if(isset($_GET['member']))
        {
            $VIEW_DATA['member'] = $_GET['member'];
        }
        $this->view->render('dsp_search_total', $VIEW_DATA, $this->theme_code);
        exit();
    }
    public function dsp_guide()
    {
        $VIEW_DATA = $this->get_default_data();
        
        $this->view->render('dsp_guide', $VIEW_DATA, $this->theme_code);
        exit();
    }
    public function dsp_signup_hoso()
    {
        $VIEW_DATA = $this->get_default_data();
        
        $this->view->render('dsp_signup_hoso', $VIEW_DATA, $this->theme_code);
        exit();
    }
    public function dsp_question()
    {
//        $VIEW_DATA = $this->get_default_data();
        
        $this->view->render('dsp_question', $VIEW_DATA, $this->theme_code);
        exit();
    }
    public function dsp_list_unit()
    {
//        $VIEW_DATA = $this->get_default_data();
        
        $this->view->render('dsp_list_unit', $VIEW_DATA, $this->theme_code);
        exit();
    }
    ...
    public function dsp_single_notification()
    {
        $VIEW_DATA = $this->get_default_data();
        $this->view->render('dsp_single_notification', $VIEW_DATA, $this->theme_code);
    }
    public function dsp_notification()
    {
        $VIEW_DATA = array();
        $this->view->render('dsp_single_article', $VIEW_DATA, $this->theme_code);
    }
//    ############################# Chưa làm đến ###########################
    ...
    {
        $VIEW_DATA['website_id'] = $this->website_id;
...
//        header("Refresh: 0; url=$page");
    ...
    #################Start Van ban ###########################
    public function dsp_all_van_ban_phap_quy()
    {
        $VIEW_DATA = $this->get_default_data();
        $this->view->active_menu_top = ' ';
        // get arrData
        $condition = "";
        $txt_title = get_request_var('txt_title','');
        $txt_cqbh = get_request_var('sel_cqbh','');
        $txt_lvtk = get_request_var('sel_lvtk','');
        $txt_loai_vb = get_request_var('sel_loai_vb','');
        ...
        // val search
        $VIEW_DATA['v_title'] = $txt_title;
        $VIEW_DATA['v_cqbh'] = $txt_cqbh;
        $VIEW_DATA['v_lvtk'] = $txt_lvtk;
        $VIEW_DATA['v_loai_vb'] = $txt_loai_vb;
  
        $VIEW_DATA['arrDoc'] =  $this->model->qry_all_doc($condition);
        $VIEW_DATA['arrNewDoc'] =  $this->model->qry_all_new_doc();
        ...
        $this->view->render('dsp_all_van_ban_phap_quy', $VIEW_DATA, $this->theme_code);
    }
    
    /**
     * danh sach tat ca phong su anh
     */
    public function dsp_info_document()
    {
        $pk_vb = get_post_var('txt_pk_vb', 0);
        $VIEW_DATA['title'] = get_post_var('txt_title', '');
        $VIEW_DATA['isCheck'] = get_post_var('txt_Check', 'false') == 'true' ? true : false;
        
        $VIEW_DATA['arrData'] = $singleDoc = $this->model->qry_single_doc($pk_vb);
        if(count($VIEW_DATA['arrData']) <= 0)
        {
            exit('Thông tin văn bản không hợp lệ');
        }
        
        $fk_cqbh = $singleDoc['FK_CO_QUAN_BAN_HANH'];
        $fk_lvtk = $singleDoc['FK_LINH_VUC_VAN_BAN'];
        
        $VIEW_DATA['arrDocCQBH'] = $this->model->qry_doc_involve_by_cqbh($pk_vb,$fk_cqbh);
        $VIEW_DATA['arrDocLVTK'] = $this->model->qry_doc_involve_by_lvtk($pk_vb,$fk_lvtk);
        
        $VIEW_DATA['arr_all_attachment'] = $this->model->qry_all_van_ban_attachment($pk_vb);
        
        $this->view->render('dsp_info_document', $VIEW_DATA);
    }

    #################End Van ban ###########################
    
    public function dsp_service(){
        $VIEW_DATA = array();
        $this->view->render('dsp_service', $VIEW_DATA, $this->theme_code);
    }
    
    public function dsp_mot_cua_dien_tu()
    {
        $this->view->render('dsp_mot_cua_dien_tu',array(),$this->theme_code);
    }

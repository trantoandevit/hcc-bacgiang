...
class member_Model extends Model {
    ...
    public function update_member($arr_post = array()) {
        ...
        $v_developer = $arr_post['developer'];
//        $v_rad_village      = $arr_post['rad_village'];
        $v_order = isset($_REQUEST['txt_order']) ? $_REQUEST['txt_order'] : 1;
        $v_fk_member = $arr_post['fk_member'];
        $v_fk_village = $arr_post['fk_village'];
        $v_login_url = $arr_post['login_url'];
        //kiem tra ma don vi
...
//            }
    ...
    public function get_all_record_type($id_member, $developer) {
        $stmt = "SELECT * FROM t_ps_record_type WHERE C_MEMBER_CODE = ? AND C_DEVELOPER_CODE = ? AND C_LEVEL = '3'";
        return $this->db->GetAll($stmt, array($id_member, $developer));
    }

    public function get_all_record_type_member($id_member, $developer) {
        $stmt = "SELECT
                t_ps_record_type_member.FK_RECORD_TYPE,
                t_ps_record_type_member.FK_MEMBER_CODE,
                t_ps_record_type.C_CODE,
                t_ps_record_type.C_STATUS,
                t_ps_record_type.C_ORDER,
                t_ps_record_type.C_NAME,
                t_ps_record_type_member.C_DEVELOPER_CODE
                FROM
                t_ps_record_type
                INNER JOIN t_ps_record_type_member ON t_ps_record_type.C_DEVELOPER_CODE = t_ps_record_type_member.C_DEVELOPER_CODE 
                AND t_ps_record_type.PK_RECORD_TYPE = t_ps_record_type_member.FK_RECORD_TYPE
                WHERE t_ps_record_type_member.FK_MEMBER_CODE = ? AND t_ps_record_type_member.C_DEVELOPER_CODE = ?
                ORDER BY t_ps_record_type.C_ORDER";
        return $this->db->GetAll($stmt, array($id_member, $developer));
    }

    public function insert_record_type_member($code, $id_member, $developer) {

        $stmt = "INSERT INTO t_ps_record_type_member (FK_MEMBER_CODE,FK_RECORD_TYPE,C_DEVELOPER_CODE) VALUES ('$code','$id_member','$developer')";
        return $this->db->Execute($stmt);
    }

    public function get_all_spec($id_member, $developer) {
        $stmt = "SELECT * FROM t_ps_spec WHERE C_MEMBER_CODE = ? and C_DEVELOPER_CODE = ? ";
        return $this->db->GetAll($stmt, array($id_member, $developer));
    }
...
//    }
    ...
    function del_record_type_member() {
        $dsp_single = get_post_var('hdn_dsp_single_method');
        $arr_item = isset($_POST['chk-item']) ? $_POST['chk-item'] : array();
        foreach ($arr_item as $row)
        {
            $sql = "Delete From t_ps_record_type_member Where FK_RECORD_TYPE In($row)";
            $this->db->Execute($sql);
        }
//        $this->exec_done($this->goback_url . $dsp_single, array());
        if ($this->db->ErrorNo() == 0) {
            $this->exec_done($this->goback_url . $dsp_single, array());
        }
        
        $this->exec_fail($this->goback_url . $dsp_single, 'Đã xảy ra lỗi. Xin vui lòng thử lại!');
    }

...
//        $html .= '<input type="button" class="ButtonSyn" onClick="btn_syn_onclick();" value="' . __('synchronize') . '"></input>';
        $html .= '<input type="button" class="ButtonSyn" onClick="btn_syn_member_onclick();" value="Đồng bộ thành viên"></input>';
...
<form name="frmMain" id="frmMain" action="" method="POST">
    ...
    echo $this->hidden('hdn_syn_member_method', 'do_sync_member');
    ?>
...
<script>
    ...
    function btn_syn_member_onclick()
    {
        if (confirm('Bạn chắc chắn đồng bộ hóa dữ liệu !!!'))
        {
            var f = document.frmMain;
            m = $("#controller").val() + f.hdn_syn_member_method.value;
            $('#frmMain').attr('action', m);
            f.submit();
        }
    }

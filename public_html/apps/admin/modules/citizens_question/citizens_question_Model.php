...
class citizens_question_Model extends Model {
    ...
    public function qry_all_question()
    {
        ...
        $sql="select q.PK_CQ,
                ...
                q.C_STATUS,
                q.C_ORDER
            from t_ps_cq q left join t_ps_cq_field f on f.PK_FIELD = q.FK_FIELD
            where 1=1 AND (f.FK_WEBSITE = $v_website_id $sql_search OR q.FK_FIELD IS NULL)";
    ...
    public function qry_single_question($id)
    {
        ...
        if(is_numeric($id))
        {
            $stmt="select COUNT(*) from (
                        select FK_WEBSITE from t_ps_cq q left join t_ps_cq_field f 
                        on q.FK_FIELD = f.PK_FIELD
                        where q.PK_CQ = $id and (f.FK_WEBSITE = $v_website_id  OR q.`FK_FIELD` IS NULL)) a ";
            $count = $this->db->GetOne($stmt);
